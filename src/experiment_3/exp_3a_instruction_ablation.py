...
if __name__ == '__main__':
    # Step 0. Load in experimental parameters
    ...
    """Task Instruction Dataset Creation"""

    # begin building varied task instructions
    task_instruction_template_types = [
        "rp",
        "minimal",
        "mistral",
        "llama-7b",
    ...
    instruction_list = []
    for instruction_ex in task_instruction_template_types:
        instruction_list.append(get_instruction_template(instruction_ex))

    instruction_dict = dict(map(lambda i, j: (i, j),
                                task_instruction_template_types, instruction_list))
    ...
    for i in range(3):
        # collect icl examples from full id_dataset
        ...
        prompts_df = Prompt(id_dist_icl_exs_ds).create_prompts()
        ...
        for instruct_name in instruction_dict.keys():
            # id test set
            ...
            id_test_df["fully_assembled_prompts"] = (instruction_dict[instruct_name] + " \n"
                                                     + id_test_df["prompts"]
            ...
            ID_OUTPUT_FILE_PATH = (OUTPUT_DIR + model_id + "_" + instruct_name +
                                   "_id_task_" + str(i) + ".csv")
            ...
            ood_test_df["fully_assembled_prompts"] = (instruction_dict[instruct_name] + " \n"
                                                     + ood_test_df["prompts"]
                                                     + "### Natural Language Instruction: \n"
            ...
            OOD_OUTPUT_FILE_PATH = (OUTPUT_DIR + model_id + "_" + instruct_name +
                                    "_ood_task_ablation_" + str(i) + ".csv")
