...
Selma, TODO:
    + include preambles // original task instructions
    + id, test-set queries
    + ood, test-set queries
    + ood, adversarial test-set queries
"""
...
class Prompt:
    def __init__(self, id_ds: Dataset):
    ...
    def create_robert_prompt(self, new_instruction: str) -> list:
        ...
        # PERMUTATIONS IN COMMENTED LINE BELOW:
        # k_combination_examples = self.create_k_many_prompt(robert_prompts, 2)
        k_combination_examples = self.create_k_many_combination_prompt(robert_prompts, 3)
    ...
    def create_naive_prompt(self) -> list:
        ...
        for batch_idx, ex in enumerate(self.id_input_information):
            ...
            naive_prompts.append("### Natural Language Instruction: \n" + nl_instruction + ' + ' +
                                 visual_info + '\n' + "### JSON format: \n" + umrf_graph)
        # Create k-combinatorial examples
        # k_combination_examples = self.create_k_many_prompt(naive_prompts, 3)
        k_combination_examples = self.create_k_many_combination_prompt(naive_prompts, 3)
